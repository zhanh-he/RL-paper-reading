...
`主题` 严格使用两个标签：第一个是领域（`MusicGen / MusicEval / MIR / AudioGen / SpeechEnhance / AudioLLM / LLM / CV / ML / SourceSep / Multimodal / Other`），第二个是项目分工（`Reward / RL / Both / Other`）。`Reward` 包括指标、评估器、reward model、confidence 与 benchmark；`RL` 包括优化器和 post-training 算法；`Both` 表示两者都是论文核心；`Other` 表示底座、背景或基础设施。
...
| 2026<br>arXiv |  | 5/5 | `MusicEval` · `Both` | MuseCritic: Learning Multi-Aspect Song Rewards through Natural-Language Aesthetic Critiques | MuseCritic；critique-then-score；multi-aspect reward | [paper](https://arxiv.org/abs/2608.11755) · [github](https://github.com/WuqnEl/MuseCritic) | 五维 learned song reward；Music Arena 71.35% 是 overall preference 而非 beat。与 explicit BeatReward 互补，不能用 Musicality/Coherence 替代 rhythm-specific labels。 |
...
| 2026<br>arXiv |  | 4.5/5 | `SpeechEnhance` · `Both` | FlowSE-GRPO: Training Flow Matching Speech Enhancement via Online Reinforcement Learning | FlowSE-GRPO；single-metric hacking；multi-metric reward | [paper](https://arxiv.org/abs/2601.16483) | 单独优化 DNSMOS、speaker similarity 或 SpeechBERTScore 会快速提分但损害其他 fidelity；证明 GRPO 能强推 proxy，真正风险是 policy-induced hacking。 |
...
| 2025<br>NeurIPS |  | 4.5/5 | `MusicGen` · `Both` | LeVo: High-Quality Song Generation with Multi-Preference Alignment | LeVo；dual-track tokens；multi-preference DPO | [paper](https://arxiv.org/abs/2506.07520) · [demo](https://levo-demo.github.io/) | 最接近 song-level multi-preference DPO；可借鉴多维偏好拆分与 dual-track tokens；官方 GitHub 于 2026-09-02 返回 404，故暂不列死链接。 |
| 2025<br>arXiv |  | 4.5/5 | `MusicGen` · `Both` | Towards Hallucination-Free Music: A Reinforcement Learning Framework for Hallucination Control | PER reward；hallucination control；DPO/PPO/GRPO | [paper](https://arxiv.org/abs/2508.05011) | same-prompt PER filtering 与 DPO/PPO/GRPO 对照；objective reward 和 subjective lyric consistency 未完全同向，直接支撑 human guardrail。 |
| 2023<br>ICML |  | 4.5/5 | `LLM` · `Both` | Scaling Laws for Reward Model Overoptimization | proxy/gold divergence；Best-of-N；scaling law | [paper](https://proceedings.mlr.press/v202/gao23h.html) | Best-of-N 与 RL 都可能随优化压力增加而偏离 gold；要求 Best-of-K 同时报告 proxy、control 与 human 曲线。 |
| 2024<br>ICLR |  | 4.5/5 | `LLM` · `Both` | Reward Model Ensembles Help Mitigate Overoptimization | worst-case ensemble；uncertainty weighting；conservative RLHF | [paper](https://proceedings.iclr.cc/paper_files/paper/2024/hash/dda7f9378a210c25e470e19304cce85d-Abstract-Conference.html) | worst-case/uncertainty-weighted ensembles 配合小 KL 可缓解 overoptimization；共享 blind spot 仍需攻击测试。 |
...
| 2024<br>arXiv |  | 4.5/5 | `LLM` · `Both` | Uncertainty-Penalized RLHF with Diverse Reward LoRA Ensembles | UP-RLHF；reward LoRA ensemble；uncertainty penalty | [paper](https://arxiv.org/abs/2401.00243) | 支持用异构 reward ensemble 估计不确定性并保守优化；tracker 投票仍不等于正确。 |
| 2024<br>CoLM |  | 4.5/5 | `LLM` · `Both` | Helping or Herding? Reward Model Ensembles Mitigate but Do Not Eliminate Reward Hacking | shared blind spots；ensemble hacking；reward diversity | [paper](https://arxiv.org/abs/2312.09244) | ensemble 可缓解却不能消除 reward hacking；Madmom + Beat This 只能提供 uncertainty evidence。 |
...
| 2026<br>Web |  | 4/5 | `LLM` · `Both` | Awesome RLVR Literature Review and Resource Registry | RLVR；verifiable reward；codebase registry | [github](https://github.com/opendilab/awesome-RLVR) · [web](https://rlvrbook.com/) | 把 music reward receipt 放入 hard checks + semi-verifiable MIR + learned rewards + human validation 的统一 RLVR 图谱。 |
...
| 2026<br>arXiv |  | 4/5 | `AudioGen` · `Both` | Resonate: Reinforcing Text-to-Audio Generation via Online Feedback from Large Audio Language Models | Resonate；LALM feedback；online GRPO | [paper](https://arxiv.org/abs/2603.11661) | 说明 reward 成熟时 online GRPO 可优于 offline；不支持“DPO 普遍优于 GRPO”，反而强调先修 reward。 |
